...
const branchName = exec('git rev-parse --abbrev-ref HEAD', { trim: true }),
    ...
    isInRemote =
        exec(`git show-branch remotes/origin/${branchName}`, {
            toString: false,
        }).code === 0;
...
const FILES_REGEX = { ts: /\.ts$/, spec: /\.spec\.ts$/ },
    ...
    forbiddenTokens = {
        ...
        '.skip': {
            rgx: /(describe|context|it)\.skip/,
            fileRgx: FILES_REGEX.spec,
        },
        '.only': {
            rgx: /(describe|context|it)\.only/,
            fileRgx: FILES_REGEX.spec,
        },
...
for (let [term, value] of Object.entries(forbiddenTokens)) {
    ...
    if (failedFiles.length > 0) {
        const msg =
            message || `The following files contains '${term}' in them:`;
        console.log(import('chalk').bgRed.black.bold(msg));
